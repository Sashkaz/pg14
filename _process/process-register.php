<?php
    ...
    if(isset($_POST) && !empty($_POST)){
        ...
        if(($response = $createAccount->userRegistration($inputArray)) == 1){
            ...
            $sql = "INSERT INTO 
                    user    (firstName, lastName, email, password, profilePicUrl, dob, gender, publicID, accountStatusID, lastLogin, genderPreference) 
                    values  ('$sanitizedInput[1]', '$sanitizedInput[2]', '$sanitizedInput[3]', '$sanitizedInput[4]', 'null', '$sanitizedInput[6]', '$sanitizedInput[7]', '".microtime()."', '1', 'null', '0')";
                    //check if typed email already exists
                    if(mysqli_num_rows($db->q($sql)) == 0){
                        if($db->q($sql)){
                            //$_SESSION["uid"] = $db->db->insert_id;
                            //$_SESSION["full_name"] = $sanitizedInput[1]. " ".$sanitizedInput[2];
                            header("Location: index.php");
                        }else{
                            echo "something went wrong ".mysqli_error($db->db);
                        }
                    }else{
